...
import { shouldVerifySupabaseSession } from "./proxy-policy";

export { shouldVerifySupabaseSession } from "./proxy-policy";

export async function updateSupabaseSession(request: NextRequest) {
  if (!shouldVerifySupabaseSession(request)) return NextResponse.next({ request });
