...
const scanResultSchema = z.object({
  ...
  githubPainEvidenceAlignment: z.object({
    inspectedCount: z.number().int().nonnegative(),
  ...
  githubFeatureEvidenceAlignment: z.object({
    ...
    mismatches: z.array(z.object({
      conversationId: z.string().uuid(),
      queryPlanId: z.string().trim().min(1).max(180),
      featureMarkers: z.array(z.string().trim().min(1).max(80)).max(12),
      categoryMatches: z.array(z.string().trim().min(1).max(120)).max(12),
      reason: z.literal("feature_evidence_mismatch"),
    })).max(100),
  }).optional(),
  githubJobEvidenceAlignment: z.object({
    inspectedCount: z.number().int().nonnegative(),
    alignedCount: z.number().int().nonnegative(),
    mismatchCount: z.number().int().nonnegative(),
    missingAlignmentProvenanceCount: z.number().int().nonnegative(),
    missingAlignmentProvenanceReason: z.literal("missing_alignment_provenance").optional(),
    subreasonCounts: z.object({
      employment_mismatch: z.number().int().nonnegative(),
      maintainer_mismatch: z.number().int().nonnegative(),
      sparse_or_generic_mismatch: z.number().int().nonnegative(),
      buyer_context_missing: z.number().int().nonnegative(),
      category_context_missing: z.number().int().nonnegative(),
    }),
    mismatches: z.array(z.object({
      conversationId: z.string().uuid(),
      queryPlanId: z.string().trim().min(1).max(180),
...
export function selectScanCandidates(input: { conversations: ConversationRow[]; sourceById: Map<string, SourceItemRow>; max: number; provenance?: ScanDiscoveryProvenance[] }) {
  ...
  for (const conversation of input.conversations) {
    ...
    if (source.source_key === "github") {
      const jobProvenance = provenanceEntries
        .filter((entry) => entry.source === "github" && entry.demandSurface === "job_demand")
        .map((entry) => ({ queryPlanId: entry.queryPlanId, semanticQuery: entry.semanticQuery, concepts: entry.concepts }));
      const jobAlignment = alignGithubJobEvidence({
        conversationId: conversation.id,
        title: source.title ?? conversation.title,
        body: source.body ?? conversation.body ?? "",
        queries: jobProvenance,
      });
      if (jobAlignment) {
        githubJobEvidenceAlignment.inspectedCount += 1;
        if (jobAlignment.missingAlignmentProvenance) {
          githubJobEvidenceAlignment.alignedCount += 1;
          githubJobEvidenceAlignment.missingAlignmentProvenanceCount += 1;
          githubJobEvidenceAlignment.missingAlignmentProvenanceReason = "missing_alignment_provenance";
        } else if (!jobAlignment.aligned) {
          githubJobEvidenceAlignment.mismatchCount += 1;
          for (const mismatch of jobAlignment.mismatches) {
            githubJobEvidenceAlignment.subreasonCounts[mismatch.subreason] += 1;
            if (githubJobEvidenceAlignment.mismatches.length < 100) {
              githubJobEvidenceAlignment.mismatches.push(mismatch);
            }
          }
          const hasAlternateDiscoveryPath = provenanceEntries.some((entry) => entry.source !== "github" || entry.demandSurface !== "job_demand");
          if (!hasAlternateDiscoveryPath) {
            jobEvidenceSuppressedCount += 1;
            continue;
          }
        } else {
          githubJobEvidenceAlignment.alignedCount += 1;
      ...
      const featureProvenance = provenanceEntries
        ...
        .map((entry) => ({ queryPlanId: entry.queryPlanId, semanticQuery: entry.semanticQuery, concepts: entry.concepts }));
      const featureAlignment = alignGithubFeatureEvidence({
        conversationId: conversation.id,
        title: source.title ?? conversation.title,
        body: source.body ?? conversation.body ?? "",
        queries: featureProvenance,
      });
      if (featureAlignment) {
        githubFeatureEvidenceAlignment.inspectedCount += 1;
        if (!featureAlignment.aligned) {
          githubFeatureEvidenceAlignment.mismatchCount += 1;
          githubFeatureEvidenceAlignment.mismatches.push(...featureAlignment.mismatches.slice(0, 100 - githubFeatureEvidenceAlignment.mismatches.length));
          const hasAlternateDiscoveryPath = provenanceEntries.some((entry) => entry.source !== "github" || entry.demandSurface !== "feature_demand");
          if (!hasAlternateDiscoveryPath) {
            featureEvidenceSuppressedCount += 1;
            continue;
          }
        }
        if (featureAlignment.aligned) githubFeatureEvidenceAlignment.alignedCount += 1;
      }
      ...
      const painProvenance = provenanceEntries
        .filter((entry) => entry.source === "github" && entry.demandSurface === "pain_first")
        .map((entry) => ({ queryPlanId: entry.queryPlanId, githubPainRetrievalV1: entry.githubPainRetrievalV1 }));
      const alignment = alignGithubPainEvidence({
        conversationId: conversation.id,
        title: source.title ?? conversation.title,
        body: source.body ?? conversation.body ?? "",
        queries: painProvenance,
      });
      if (alignment) {
        githubPainEvidenceAlignment.inspectedCount += 1;
        if (!alignment.aligned) {
          githubPainEvidenceAlignment.mismatchCount += 1;
          githubPainEvidenceAlignment.mismatches.push(...alignment.mismatches.slice(0, 100 - githubPainEvidenceAlignment.mismatches.length));
          continue;
        }
        githubPainEvidenceAlignment.alignedCount += 1;
      }
