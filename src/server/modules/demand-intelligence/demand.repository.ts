...
export interface DemandRepository {
  ...
  /** Bounded (single row): when demand intelligence was first persisted for this product. Layer 9B's Drift v2 monitoring-history input; never a full history scan. */
  getEarliestSnapshotTimestamp(workspaceId: string, productId: string): Promise<string | null>;
...
export class SupabaseDemandRepository implements DemandRepository {
  ...
  async getEarliestSnapshotTimestamp(workspaceId: string, productId: string) { const { data, error } = await this.client.from("demand_snapshots").select("created_at").eq("workspace_id", workspaceId).eq("product_id", productId).order("created_at", { ascending: true }).limit(1).maybeSingle(); if (error) throw dbError(error, "Earliest demand snapshot timestamp could not be loaded."); return data?.created_at ?? null; }
...
export class InMemoryDemandRepository implements DemandRepository {
  ...
  async getEarliestSnapshotTimestamp(workspaceId: string, productId: string) { const rows = [...this.snapshots.values()].filter((r) => r.workspace_id === workspaceId && r.product_id === productId).sort((a, b) => a.created_at.localeCompare(b.created_at)); return rows[0]?.created_at ?? null; }
