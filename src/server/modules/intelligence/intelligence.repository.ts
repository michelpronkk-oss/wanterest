...
export interface IntelligenceRepository {
  ...
  listConversations(conversationIds: string[]): Promise<ConversationRow[]>;
  getSourceItem(sourceItemId: string): Promise<SourceItemRow | null>;
  listSourceItems(sourceItemIds: string[]): Promise<SourceItemRow[]>;
  ...
  listConversationAnalyses(analysisIds: string[]): Promise<ConversationAnalysisRow[]>;
...
export class SupabaseIntelligenceRepository implements IntelligenceRepository {
  ...
  async listConversations(conversationIds: string[]) { if (!conversationIds.length) return []; const { data, error } = await this.client.from("conversations").select("*").in("id", conversationIds); if (error) throw dbError(error, "Conversations could not be loaded."); return data ?? []; }
  ...
  async listSourceItems(sourceItemIds: string[]) { if (!sourceItemIds.length) return []; const { data, error } = await this.client.from("source_items").select("*").in("id", sourceItemIds); if (error) throw dbError(error, "Source items could not be loaded."); return data ?? []; }
  ...
  async listConversationAnalyses(analysisIds: string[]) { if (!analysisIds.length) return []; const { data, error } = await this.client.from("conversation_analysis").select("*").in("id", analysisIds); if (error) throw dbError(error, "Conversation analyses could not be loaded."); return data ?? []; }
