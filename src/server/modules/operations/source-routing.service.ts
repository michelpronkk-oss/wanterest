...
const businessTypeBase: Record<string, Record<string, number>> = {
  b2b_saas: { "hacker-news": 0.68, github: 0.58, reddit: 0.84, x: 0.72, bluesky: 0.45 },
  developer_tool: { "hacker-news": 0.88, github: 0.94, reddit: 0.78, x: 0.74, bluesky: 0.38 },
  consumer_software: { "hacker-news": 0.2, github: 0.12, reddit: 0.86, x: 0.82, bluesky: 0.55 },
  ecommerce: { "hacker-news": 0.1, github: 0.05, reddit: 0.9, x: 0.82, bluesky: 0.45 },
  marketplace: { "hacker-news": 0.45, github: 0.3, reddit: 0.88, x: 0.84, bluesky: 0.5 },
  service_business: { "hacker-news": 0.1, github: 0.03, reddit: 0.62, x: 0.68, bluesky: 0.38 },
  local_business: { "hacker-news": 0.03, github: 0.02, reddit: 0.55, x: 0.48, bluesky: 0.28 },
  agency: { "hacker-news": 0.42, github: 0.2, reddit: 0.68, x: 0.82, bluesky: 0.5 },
  media_content: { "hacker-news": 0.2, github: 0.02, reddit: 0.74, x: 0.9, bluesky: 0.72 },
  other: { "hacker-news": 0.25, github: 0.2, reddit: 0.55, x: 0.55, bluesky: 0.35 },
};
...
function initialScore(sourceKey: string, identity: ReturnType<typeof effectiveIdentity>, input: SourceRoutingInput): number {
  ...
  if (technicalAudience && (sourceKey === "github" || sourceKey === "hacker-news")) score += sourceKey === "github" ? 0.08 : 0.05;
  if (consumerAudience && (sourceKey === "reddit" || sourceKey === "x")) score += 0.04;
  ...
  if (intents.has("alternative_search") || intents.has("switching_intent") || intents.has("renewal_reconsideration")) {
    ...
    if (sourceKey === "github" && technicalAudience) score += 0.04;
  ...
  if (intents.has("comparison_intent")) {
    ...
    if (sourceKey === "github" && technicalAudience) score += 0.03;
    if (capability) score += capability.supports_competitor_comparison * 0.02;
  }
  if ((profile?.pains.length ?? 0) > 0) score += ({ reddit: 0.05, x: 0.04, bluesky: 0.03, "hacker-news": 0.03, github: technicalAudience ? 0.04 : 0 }[sourceKey] ?? 0);
  if ((profile?.jobs_to_be_done.length ?? 0) > 0 && capability) score += capability.supports_problem_discussion * 0.02;
  if ((profile?.feature_demands.length ?? 0) > 0) score += ({ github: technicalAudience ? 0.05 : 0, reddit: 0.03, x: 0.03 }[sourceKey] ?? 0);
  if ((profile?.known_competitors.length ?? 0) > 0 || (profile?.alternative_solutions.length ?? 0) > 0) score += ({ reddit: 0.03, x: 0.03, "hacker-news": 0.02 }[sourceKey] ?? 0);

  if (identity.locationDependency >= 0.8 || identity.marketScope === "local") {
    if (sourceKey === "hacker-news" || sourceKey === "github") return 0;
  ...
  if (identity.businessType === "ecommerce" && (sourceKey === "hacker-news" || sourceKey === "github")) return 0;
  if (identity.businessType === "local_business" && (sourceKey === "hacker-news" || sourceKey === "github")) return 0;
...
function availability(state: SourceRoutingSourceState | undefined): { availabilityStatus: SourceRoutingAvailabilityStatus; healthStatus: SourceRoutingRoute["health_status"] } {
  if (!state || !state.configured) return { availabilityStatus: "not_configured", healthStatus: state?.healthStatus ?? "unknown" };
...
function reasonCodes(sourceKey: string, score: number, identity: ReturnType<typeof effectiveIdentity>, input: SourceRoutingInput, sourceState: SourceRoutingSourceState | undefined, healthStatus: SourceRoutingRoute["health_status"]): SourceRoutingReasonCode[] {
  ...
  if (technical && sourceKey === "github") codes.push("DEVELOPER_AUDIENCE_MATCH");
  if (technical && (sourceKey === "github" || sourceKey === "hacker-news")) codes.push("TECHNICAL_DISCUSSION_MATCH");
  const intents = new Set(profile?.buying_intents.map((intent) => intent.intent_type) ?? []);
  ...
  if ((identity.businessType === "consumer_software" || identity.businessModel === "b2c") && (sourceKey === "reddit" || sourceKey === "x")) codes.push("CONSUMER_DISCUSSION_MATCH");
  if (identity.businessType === "ecommerce" && (sourceKey === "reddit" || sourceKey === "x")) codes.push("ECOMMERCE_MATCH");
  if (identity.locationDependency >= 0.8 || identity.marketScope === "local") {
    codes.push("HIGH_LOCATION_DEPENDENCY");
    if (sourceKey !== "reddit") codes.push("LOCAL_COVERAGE_WEAK");
  }
...
function defaultCap(sourceKey: string, scanMode: SourceRoutingScanMode): { maxCandidates: number; maxPages: number } {
  if (scanMode === "onboarding" || scanMode === "baseline") return { maxCandidates: sourceKey === "x" ? 10 : 5, maxPages: 1 };
  return { maxCandidates: sourceKey === "x" ? 25 : 25, maxPages: sourceKey === "x" ? 1 : 3 };
}

function minimumCandidateBudget(route: SourceRoutingRoute, scanMode: SourceRoutingScanMode): number {
  if (scanMode === "manual" || scanMode === "manual_refresh") {
    return ({ x: 6, github: 8, "hacker-news": 4, bluesky: 6 } as Record<string, number>)[route.source_key] ?? 1;
  }
...
function allocateBudget(routes: SourceRoutingRoute[], totalBudget: number, maxSources: number, input: SourceRoutingInput): { routes: SourceRoutingRoute[]; selected: SourceRoutingRoute[] } {
  ...
  const candidates = routes
    ...
    .sort((a, b) => b.relevance_score - a.relevance_score || a.source_key.localeCompare(b.source_key));
...
export function buildSourceRoutingPlan(input: SourceRoutingInput): SourceRoutingPlan {
  ...
  const maxSources = input.maxSources ?? ((input.scanMode === "onboarding" || input.scanMode === "baseline") ? 3 : input.scanMode === "manual" || input.scanMode === "manual_refresh" ? 4 : routes.length);
  ...
  const relevant = finalRoutes.filter((route) => route.enabled_for_product);
